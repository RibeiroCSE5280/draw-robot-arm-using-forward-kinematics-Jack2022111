...
def main():
    ...
    axes = Axes(xrange=(0, 25), yrange=(-2, 20), zrange=(0, 6))

    r = 0.4  # Radius for sphere joints
    ...
    p2 = np.array([[L1 + (2 * r)], [0.0], [0.0]])  # Frame's origin (w.r.t. previous frame)
    ...
    p3 = np.array([[L2 + (2 * r)], [0.0], [0.0]])  # Frame's origin (w.r.t. previous frame)
    ...
    # End-effector Matrix (written w.r.t. Frame 3, which is the previous frame)
    R_34 = RotationMatrix(phi4, axis_name='z')
    p4 = np.array([[L3 + (2 * r)], [0.0], [0.0]])
    t_34 = p4
    ...
    # Also create a sphere to show as an example of the end of the joint
    sphere4 = Sphere(r=r).pos(0, 0, 0).color("black").alpha(1)
    ...
    # Show everything
    show([box_mesh, Frame1, Frame2, Frame3, end_effector], axes, viewup="z").close()
